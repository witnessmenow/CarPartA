...
public class TeamSelectArea {
	...
	private final float playerGapX;
	private final float playerGapY;
	...
	public TeamSelectArea(World world, float worldWidth, float worldHeight,
			Vector2 center, float gapFromOuterEdge, float playerGapX,
			float playerGapY) {
		this.center = center;
		...
		createOuterWalls(world, worldWidth, worldHeight, center,
				gapFromOuterEdge);
		
		createInnerWalls(world, worldWidth, worldHeight, center,
				gapFromOuterEdge);
			...
			float worldHeight, Vector2 center, float gapFromOuterEdge)
	{
		...
		// outer walls
		// bottomWall
		new BoxProp(world, worldWidth, 1, new Vector2(worldWidth / 2,
				gapFromOuterEdge));
		
		// leftWall
		new BoxProp(world, 1, worldHeight / 2, new Vector2(gapFromOuterEdge,
				worldHeight / 4));
		...
		// rightWall
		new BoxProp(world, 1, worldHeight / 2, new Vector2(worldWidth
				- gapFromOuterEdge, worldHeight / 4));
			...
			float worldHeight, Vector2 center, float gapFromOuterEdge)
	{
		...
		// leftInnerWall
		new BoxProp(world, 1, worldHeight / 4, new Vector2((worldWidth / 4),
				3 * (worldHeight / 8)));
		
		// rightInnerWall
		new BoxProp(world, 1, worldHeight / 4, new Vector2(
				3 * (worldWidth / 4), 3 * (worldHeight / 8)));
	...
	public StartingPosition getStartPosition(int carNumber)
	{
		...
		if (carNumber % 2 == 0)
		{
			carPosX = this.center.x - ((carNumber / 2) + 1) * playerGapX;
		}
		else
		{
			carPosX = this.center.x + ((carNumber / 2) + 1) * playerGapX;
		...
		return new StartingPosition(new Vector2(carPosX, this.center.y
				- playerGapY), 0f);
		
	}
	
	// This is not the fanciest way of doing this, a contact listener would be
	// But I can't think of a good reason why it would be better, in fact its
	// more complicated!
			...
			int aIHome, int aIAway)
	{
		...
		if (aIHome > 0)
		{
			...
			font.setColor(0.3f, 0.5f, 1f, 1.0f);
			font.draw(spriteBatch, blueMessage,
					homeTextX - font.getBounds(blueMessage).width / 2, aIYText);
		}
		...
		if (aIAway > 0)
		{
			...
			font.setColor(1f, 0f, 0f, 1.0f);
			font.draw(spriteBatch, redMessage,
					awayTextX - font.getBounds(redMessage).width / 2, aIYText);
		}
	}
	
	public void displayMessageInSelectArea(BitmapFont font,
			SpriteBatch spriteBatch, String message)
	{
		font.setColor(1.0f, 1.0f, 1.0f, 1.0f);
		font.draw(spriteBatch, message,
				(worldWidth / 2) * 10 - font.getBounds(message).width / 2,
				(worldHeight / 5) * 10);
	}
