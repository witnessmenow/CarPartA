...
public class PongCollisionHelper extends AbstractCollisionHelper {
	...
	public void beginContact(Contact contact)
	{
		...
		if (bodyAInfo != null && bodyBInfo != null)
		{
			...
			if (CollisionHelper.checkIfCollisionIsOfCertainBodies(bodyAInfo,
					bodyBInfo, CollisionObjectType.Wall,
					CollisionObjectType.BallSensor))
			{
			...
			else if (CollisionHelper.checkIfCollisionIsOfCertainBodies(
					bodyAInfo, bodyBInfo, CollisionObjectType.Vehicle,
					CollisionObjectType.BallSensor))
			{
				Vehicle v;
				PongBall pb;
				...
				{
					if ((pb.body.getWorldCenter().y + pb.ballSize / 2) >= (v.body
							.getWorldCenter().y - v.length / 2 - 0.5f))
					{
						pb.carHit();
					}
					else
				...
				else
				{
					...
					bodyAInfo, bodyBInfo, CollisionObjectType.ScoreZone,
					CollisionObjectType.BallSensor))
			{
				// Goal Scored
