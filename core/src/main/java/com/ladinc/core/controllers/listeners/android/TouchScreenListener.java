			...
			boolean touched, boolean moveEvent)
	{
		if (screenY > screenHeight / 2)
		{
			...
			if (screenX <= 0)
			{
				...
				Gdx.app.debug("TouchScreenListener", "setLeftButton: touched="
			...
			if (screenX <= 0)
			{
				...
				Gdx.app.debug("TouchScreenListener", "setRightButton: touched="
						+ String.valueOf(touched));
		...
		else
		{
			if (!moveEvent && screenX <= (this.screenWidth / 4))
			{
				...
				return;
					...
					&& (screenX > (this.screenWidth / 4) && screenX < 3 * (this.screenWidth / 4)))
			{
