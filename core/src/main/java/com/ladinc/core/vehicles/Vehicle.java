...
public class Vehicle {
	...
	public void createBody(World world)
	{
		...
		this.body.setUserData(new CollisionInfo("",
				CollisionObjectType.Vehicle, this));
	...
	public void createWheels()
	{
		...
		this.wheels.add(new Wheel(world, this, -(wheelXPos), (-wheelYPos),
				wheelWidth, wheelLength, true, true, wheelSprite)); // top left
		this.wheels.add(new Wheel(world, this, (wheelXPos), (-wheelYPos),
				wheelWidth, wheelLength, true, true, wheelSprite)); // top right
		this.wheels.add(new Wheel(world, this, -(wheelXPos), wheelYPos,
				wheelWidth, wheelLength, false, false, wheelSprite)); // back
																		// left
		this.wheels.add(new Wheel(world, this, (wheelXPos), wheelYPos,
				wheelWidth, wheelLength, false, false, wheelSprite)); // back
																		// right
	...
	private void createSensorBody(World world)
	{
		...
		jointdef.initialize(this.body, this.sensorBody,
				this.sensorBody.getWorldCenter(), new Vector2(1, 1));
		jointdef.enableLimit = true;
		jointdef.lowerTranslation = jointdef.upperTranslation = 0;
		world.createJoint(jointdef);
		
		this.sensorBody.setUserData(new CollisionInfo("",
				CollisionObjectType.VehicleSensor, this));
	...
	public void update(float deltaTime)
	{
		...
		if (respawnTimeRemaining > 0.0f)
		{
			...
			// The reason im doing the second check here is that car will be
			// updated more often when active
			// So it reduces the number of if statements checked on a regular
			// render
			if (respawnTimeRemaining > 0.0f)
			{
