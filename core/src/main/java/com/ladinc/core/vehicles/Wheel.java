...
public class Wheel {
	...
	private final float width; // width in meters
	private final float length; // length in meters
	...
	public Wheel(World world, Vehicle vehcile, float posX, float posY,
			float width, float length, boolean revolving, boolean powered,
			Sprite wheelSprite) {
		super();
	...
	public void createWheelBody(World world)
	{
		...
		bodyDef.position.set(vehcile.body
				.getWorldPoint(new Vector2(posX, posY)));
		...
		if (this.revolving)
		{
			...
			jointdef.initialize(this.vehcile.body, this.body,
	...
	public void setAngle(float angle)
	{
		 ...
		 */
		this.body.setTransform(body.getPosition(), this.vehcile.body.getAngle()
	...
	public Vector2 getLocalVelocity()
	{
		 ...
		 */
		return this.vehcile.body.getLocalVector(this.vehcile.body
	...
	public Vector2 getDampedVelocityVector()
	{
		...
		dotprod = dotprod - (dotprod / 100);
		return new Vector2(sidewaysAxis.x * dotprod / 2, sidewaysAxis.y
				* dotprod / 2);
	};
	...
	public void updateSprite(SpriteBatch spriteBatch, int PIXELS_PER_METER)
	{
		if (this.sprite == null)
		{
			if (!Art.spriteTable.containsKey(Art.WHEELS))
			{
				...
				Art.spriteTable.put(Art.WHEELS,
						new Sprite(Art.textureTable.get(Art.WHEELS), w, l));
			}
